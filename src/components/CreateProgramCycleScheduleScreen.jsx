...
function PlusIcon() { return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 6v12M6 12h14" /></svg>; }
...
function countForm(count, forms) { const value = Math.abs(Number(count) || 0); const m100 = value % 100; const m10 = value % 10; if (m100 >= 11 && m100 <= 14) return forms[2]; if (m10 === 1) return forms[0]; if (m10 >= 2 && m10 <= 4) return forms[1]; return forms[2]; }
function formatCount(count, forms) { return `${count} ${countForm(count, forms)}`; }
function formatDays(count) { return formatCount(count, ['день', 'дня', 'дней']); }
...
export function CreateProgramCycleScheduleScreen({
  ...
  return <div className="phone create-program-phone program-schedule-phone cycle-schedule-phone">
    ...
    <main className="create-program-content program-schedule-content">
      ...
      <section className={`program-schedule-section cycle-repeat-section${cycleRepeatLocked ? ' locked' : ''}`}><div className="program-schedule-section-head"><div><span>Повторение</span><h2>Сколько раз повторить цикл</h2></div></div><div className="cycle-repeat-card"><div><strong>{repeatCount}</strong><span>{countForm(repeatCount, ['цикл', 'цикла', 'циклов'])}</span></div><div className="cycle-repeat-stepper"><button type="button" onClick={() => changeRepeats(-1)} disabled={saving || cycleRepeatLocked || repeatCount <= 1}><MinusIcon /></button><button type="button" onClick={() => changeRepeats(1)} disabled={saving || cycleRepeatLocked || repeatCount >= 52}><PlusIcon /></button></div></div>
