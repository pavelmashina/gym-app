...
function CatalogCard({ program, onOpen }) {
  ...
  return (
    <button className="catalog-program-card" type="button" onClick={() => onOpen(program.id)}>
      <span className="catalog-program-cover catalog-program-placeholder" aria-hidden="true"><span className="catalog-placeholder-icon" /></span>
      <span className="catalog-program-card-copy">
        ...
        <span className="catalog-program-card-meta">
          <span>{program.workoutCount} трен. в цикле</span>
...
function FilterGroup({ title, values, selected, onSelect }) {
  ...
  return <div className="catalog-filter-group"><span>{title}</span><div className="catalog-filter-pills">{values.map((value) => <button className={value === selected ? 'active' : ''} key={value} type="button" onClick={() => onSelect(value)}>{value}</button>)}</div></div>;
}
  ...
  return (
    <article className={`catalog-workout-card${open ? ' open' : ''}`}>
      <button className="catalog-workout-head" type="button" onClick={() => setOpen((value) => !value)}>
        ...
        <span className="catalog-workout-title"><small>Тренировка цикла</small><strong>{workout.name || `Тренировка ${index + 1}`}</strong><span>{formatRussianCount(exercises.length, ['упражнение', 'упражнения', 'упражнений'])}</span></span>
...
function InfoRow({ label, value }) {
  return <div className="catalog-info-row"><span>{label}</span><strong className={!value ? 'empty' : ''}>{value || NO_DATA}</strong></div>;
}
...
function CatalogProgramDetail({ programId, onBack, onJoin }) {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true); setError(''); setActiveTab('info');
    getCatalogProgram(programId).then((result) => { if (active) setProgram(result); }).catch((requestError) => { if (active) setError(requestError.message); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [programId]);

  const workouts = program?.sourcePayload?.cycle?.workouts ?? [];
  ...
  return (
    <div className="catalog-detail-layer" role="dialog" aria-modal="true" aria-label="Программа из каталога">
      <div className="catalog-detail-phone">
        <header className="catalog-detail-header"><button type="button" aria-label="Назад к каталогу" onClick={onBack}><BackIcon /></button><strong>Каталог программ</strong><span /></header>
        ...
        {!loading && program && <>
          <nav className="catalog-detail-tabs" aria-label="Разделы программы"><button className={activeTab === 'info' ? 'active' : ''} type="button" onClick={() => setActiveTab('info')}>О программе</button><button className={activeTab === 'workouts' ? 'active' : ''} type="button" onClick={() => setActiveTab('workouts')}>Тренировки</button></nav>
          <main className="catalog-detail-content has-footer">
            {activeTab === 'info' && <>
              <div className="catalog-detail-cover-placeholder catalog-program-placeholder" aria-hidden="true"><span className="catalog-placeholder-icon" /></div>
              <section className="catalog-detail-hero catalog-detail-hero-light"><span className="catalog-detail-kicker">{program.categories[0] || 'Готовая программа'}</span><h1>{program.name}</h1></section>
              <section className="catalog-detail-section catalog-about-section"><div className="catalog-detail-section-head"><span>Информация о программе</span></div><div className="catalog-info-card">
                ...
                <InfoRow label="Оборудование" value={program.equipment} />
                <InfoRow label="Тренировок в одном цикле" value={program.workoutCount ? String(program.workoutCount) : ''} />
                <InfoRow label="Количество повторений цикла" value="Выбирается при присоединении" />
                <InfoRow label="Продолжительность" value="Зависит от ритма и количества повторений" />
              </div></section>
            ...
            {activeTab === 'workouts' && <section className="catalog-detail-section catalog-workouts-tab-section"><div className="catalog-detail-section-head"><span>Один цикл программы</span><strong>{workouts.length ? formatRussianCount(workouts.length, ['тренировка', 'тренировки', 'тренировок']) : NO_DATA}</strong></div>{workouts.length > 0 ? <div className="catalog-workout-list">{workouts.map((workout, index) => <WorkoutBlock key={`${workout.name}-${index}`} workout={workout} index={index} />)}</div> : <div className="catalog-detail-empty-card">{NO_DATA}</div>}<div className="catalog-source-note">Один цикл можно повторить любое количество раз при присоединении. Исходные схемы подходов и повторений сохранены из загруженного каталога.</div></section>}
...
export function CatalogProgramsPanel({ query = '', sortDirection = 'asc', filtersOpen = false, onJoin }) {
  ...
  useEffect(() => {
    ...
    listCatalogPrograms().then((result) => { if (active) setPrograms(result); }).catch((requestError) => { if (active) setError(requestError.message); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [reloadKey]);
  ...
  const filteredPrograms = useMemo(() => {
    ...
    const result = programs.filter((program) => {
      ...
      const exerciseNames = (program.sourcePayload?.cycle?.workouts ?? []).flatMap((workout) => workout.exercises ?? []).map((exercise) => exercise.name);
      return [program.name, program.description, program.trainingPlace, program.equipment, program.level, ...program.categories, ...exerciseNames].filter(Boolean).some((value) => value.toLocaleLowerCase('ru').includes(normalizedQuery));
    });
    return result.sort((a, b) => { const comparison = a.name.localeCompare(b.name, 'ru'); return sortDirection === 'asc' ? comparison : -comparison; });
  ...
  return <>
    ...
    <div className="catalog-results-head"><span>{loading ? 'Загрузка…' : formatRussianCount(filteredPrograms.length, ['программа', 'программы', 'программ'])}</span>{hasFilters && <strong>Фильтр включён</strong>}</div>
    ...
    {!loading && !error && filteredPrograms.length > 0 && <section className="catalog-program-list">{filteredPrograms.map((program) => <CatalogCard key={program.id} program={program} onOpen={setSelectedProgramId} />)}</section>}
    {selectedProgramId && <CatalogProgramDetail programId={selectedProgramId} onBack={() => setSelectedProgramId(null)} onJoin={onJoin} />}
  </>;
}
