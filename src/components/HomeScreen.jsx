...
function Header({ menuOpen, onOpenMenu }) {
  return (
    <header className="appbar">
      <button className="icon-btn" type="button" aria-label="Открыть меню" aria-expanded={menuOpen} onClick={onOpenMenu}>
        <span className="burger" aria-hidden="true"><span /><span /><span /></span>
      </button>
      <button className="profile-btn" type="button" aria-label="Профиль">
        <svg className="profile-icon" viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true">
          <circle cx="16" cy="11" r="5" /><path d="M7 27c1.2-5.7 4.2-8.4 9-8.4s7.8 2.7 9 8.4" />
        </svg>
      </button>
    </header>
  );
  ...
  return (
    <section className="calendar-card">
      <div className="month-row">
        <button type="button" aria-label="Предыдущий месяц" onClick={() => changeMonth(-1)}>←</button>
        <div className="month">{formatMonth(displayMonth)}</div>
        <button type="button" aria-label="Следующий месяц" onClick={() => changeMonth(1)}>→</button>
      </div>
...
function WorkoutSummary({ workout, onOpenWorkout }) {
  ...
  return (
    <section className="summary-col workout">
      ...
      <button className="workout-btn" type="button" onClick={() => onOpenWorkout?.(workout.id)}>{buttonLabel}</button>
    </section>
  );
}
  ...
  if (!nutritionPlan) {
    return (
      <section className="summary-col food home-empty-card">
        ...
        <div className="home-empty-copy"><h3>План не выбран</h3><p>Питание пока не настроено</p></div>
      </section>
    );
  }
  return (
    <section className="summary-col food">
      ...
      <div className="food-grid">
        ...
        <div className="food-stat"><strong>{nutritionPlan.carbs} г</strong><span>Углеводы</span></div>
        <div className="food-stat"><strong>{nutritionPlan.completion ?? 0}%</strong><span>Выполнено</span></div>
      </div>
    </section>
  );
}
