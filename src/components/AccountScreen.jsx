...
function Row({ label, value }) {
  return <div className="account-data-row"><span>{label}</span><strong>{value || 'Не указано'}</strong></div>;
}
...
export function AccountScreen({ user, profile, loading, onBack, onSignOut, onAccountDeleted }) {
  ...
  return <div className="account-screen">
    <header className="account-topbar">
      <button type="button" className="account-icon-btn" onClick={onBack} aria-label="Назад"><BackIcon /></button>
      <h1>Личный кабинет</h1>
      <button type="button" className="account-icon-btn account-settings-disabled" disabled aria-label="Настройки пока недоступны" title="Настройки появятся позже"><SettingsIcon /></button>
    </header>

    <main className="account-content">
      <section className="account-identity">
        <div className="account-avatar">{avatar}</div>
        <div><h2>{displayName}</h2><span>{user?.email || 'Аккаунт'}</span></div>
      </section>

      <section className="account-card">
        <h2>Личные данные</h2>
        <Row label="Телефон" value={phone} />
        <Row label="Почта" value={user?.email || 'Не указано'} />
        <Row label="Дата рождения" value={formatDate(profile?.birth_date)} />
      </section>

      <section className="account-card">
        <h2>Ваши параметры</h2>
        <Row label="Рост" value={height} />
        <Row label="Вес" value={weight} />
        <Row label="Пол" value={sex} />
        <Row label="Уровень активности" value={activity} />
      </section>

      <section className="account-card account-plan-card">
        <h2>Мой тариф</h2>
        <button type="button" className="account-plan-link" onClick={() => setSubscreen('plan')}><div><span>Текущий тариф</span><strong>{PLAN_LABELS[planCode] || 'Базовый'}</strong></div><ArrowIcon /></button>
      </section>

      <section className="account-card account-danger-card">
        <h2>Аккаунт</h2>
        <button type="button" className="account-logout-button" disabled={loading} onClick={onSignOut}>{loading ? 'Выходим…' : 'Выйти из аккаунта'}</button>
        <button type="button" className="account-delete-button" onClick={() => { setDeleteOpen(true); setDeleteConfirm(''); setDeleteError(''); }}>Удалить аккаунт</button>
      </section>
    </main>

    {deleteOpen && <div className="account-delete-modal" role="dialog" aria-modal="true" aria-label="Удалить аккаунт"><button className="account-delete-scrim" type="button" aria-label="Закрыть" onClick={() => !deleteLoading && setDeleteOpen(false)} /><section className="account-delete-sheet"><div className="account-delete-handle" /><h2>Удалить аккаунт?</h2><p>Будут удалены профиль и связанные с аккаунтом данные. Это действие нельзя отменить.</p><label><span>Введите «УДАЛИТЬ» для подтверждения</span><input value={deleteConfirm} onChange={(event) => setDeleteConfirm(event.target.value)} autoComplete="off" /></label>{deleteError && <div className="account-delete-error">{deleteError}</div>}<button type="button" className="account-delete-confirm" disabled={deleteConfirm.trim().toUpperCase() !== 'УДАЛИТЬ' || deleteLoading} onClick={deleteAccount}>{deleteLoading ? 'Удаляем…' : 'Удалить навсегда'}</button><button type="button" className="account-delete-cancel" disabled={deleteLoading} onClick={() => setDeleteOpen(false)}>Отмена</button></section></div>}
  </div>;
}
