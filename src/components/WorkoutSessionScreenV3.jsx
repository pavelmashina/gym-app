...
function toEmbedVideoUrl(value) {
  ...
  try {
    ...
    if (host === 'youtube.com' || host === 'm.youtube.com') {
      if (url.pathname === '/watch') return url.searchParams.get('v') ? `https://www.youtube.com/embed/${url.searchParams.get('v')}` : null;
      const parts = url.pathname.split('/').filter(Boolean);
      if (parts[0] === 'shorts' || parts[0] === 'embed') return parts[1] ? `https://www.youtube.com/embed/${parts[1]}` : null;
    }
    if (host === 'vimeo.com') {
...
function ActiveWorkout({ entry, onEntryChange, onFinish, onPauseRequest, onRequestReplacement, finishing, pausing, error }) {
  ...
  function openReplacement(exercise) { setActionTarget(null); onRequestReplacement?.(exercise); }
...
export function WorkoutSessionScreen({ scheduledWorkoutId, onBack, onCompleted, onRequestReplacement, onPauseRequest, pausing = false }) {
  const [entry, setEntry] = useState(null); const [loading, setLoading] = useState(true); const [starting, setStarting] = useState(false); const [finishing, setFinishing] = useState(false); const [error, setError] = useState('');
