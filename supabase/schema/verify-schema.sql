...
begin
  ...
  if v_rpc_count <> 11 then
    raise exception 'Schema verification failed: expected 11 current SECURITY INVOKER application RPCs, found %', v_rpc_count;
  end if;

  if v_rpc_restricted_count <> 11 then
    raise exception 'Schema verification failed: authenticated/anon execute grants are correct on % of 11 application RPCs', v_rpc_restricted_count;
  end if;
  ...
  raise notice 'Schema verification passed: 16 tables, RLS/policies, 11 restricted SECURITY INVOKER RPCs, cycle model/guard, normalized catalog, snapshots/index and Storage match the repository inventory.';
end
$verify$;
